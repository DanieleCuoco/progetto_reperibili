...
<body>
    ...
    <main class="main-content">
        <div class="page-header">
            <h2><i class="bi bi-pencil"></i> Modifica Reparto</h2>
        </div>

        <div class="form-container">
            <form action="{{ route('admin.reparti.update', $reparto) }}" method="POST">
                @csrf
                @method('PUT')
                
                <div class="form-group">
                    <label for="nome">Nome Reparto</label>
                    <input type="text" id="nome" name="nome" value="{{ old('nome', $reparto->nome) }}" required>
                    @error('nome')
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="descrizione" name="descrizione" value="{{ old('descrizione', $reparto->descrizione) }}">
                    @error('descrizione')
                        <div class="error">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="codice" name="codice" value="{{ old('codice', $reparto->codice) }}" required>
                    @error('codice')
                        <div class="error">{{ $message }}</div>
                    @enderror
                </div>
                    ...
                    <div class="icon-selector">
                        @foreach($icons as $index => $icon)
                            <label class="icon-option" for="icon-{{ $index }}">
                                <input type="radio" id="icon-{{ $index }}" name="icon_name" value="{{ $icon['icon'] }}" class="icon-radio" {{ old('icon_name', $selectedIcon['icon']) == $icon['icon'] ? 'checked' : '' }}>
                ...
                <div class="form-group checkbox-group">
                    <input type="checkbox" id="is_active" name="is_active" value="1" {{ $reparto->is_active ? 'checked' : '' }}>
                    <label for="is_active">Reparto Attivo</label>
                </div>
                
                <div class="btn-group">
                    <button type="submit" class="btn-submit">
                        <i class="bi bi-check-circle"></i> Aggiorna Reparto
                    </button>
